...
function CambiaPagina(numPagine, paginaProvenienza, paginaDestinazione, dati) {    
    ...
    $.ajax({
        url: '/AziendaConsulente/Paginazione',
        dataType: "json",
        method: "POST",
...
function PageLoadSettings() {
    ...
    $(".txt_impRet").blur(function () {
        ...
        var impRet = $(this).val();
        var txtBoxImpOcc = $(this).closest("tr").children().children(".txt_impOcc");
        ...
        if (IsValued(impRet)) {
            ...
            if (IsValued(impOcc) && IsValued(minCon) && IsValued(aliq) && IsValued(impSca) && IsValued(traEco)) {
                ...
                if (impOcc > impRet && impOcc > 0) {
                    TotOccas_detrai(txtBoxImpOcc.val());
                    txtBoxImpOcc.focus();
                    $(this).focus();
                }
                else {
...
function VisualizzaSospensioni(obj) {
    ...
    var matricola = obj.parent().parent().children().eq(0).children().eq(0).val().toString();
    var proRap = obj.parent().parent().children(".proRap").html().toString();
    var dal = obj.parent().parent().children().eq(2).html().toString();
    var al = obj.parent().parent().children().eq(3).html().toString();
    var row = "";
    ...
    if (IsValued(importoCorrente) && IsValued(importoPrecedente)) {
        ...
        if (classi.includes("txt_impRet")) {
            if (IsValued(totaleImponibili)) {
                ...
                totaleImponibili = GestioneInserimento(totaleImponibili.toFixed(2));
                $("#totImponibili").html(totaleImponibili);
            }
        }
            ...
            if (IsValued(totaleOccasionali)) {
                ...
                totaleOccasionali = GestioneInserimento(totaleOccasionali.toFixed(2));
                $("#totOccasionali").html(totaleOccasionali);
            }
        }
...
function TotOccas_detrai(value) {
    
    var importoOccasionale = CleanValue(value);
    var totaleOccasionali = $("#totOccasionali").html();
    totaleOccasionali = CleanValue(totaleOccasionali);
    totaleOccasionali = totaleOccasionali - importoOccasionale;
    totaleOccasionali = GestioneInserimento(totaleOccasionali.toFixed(2));
    $("#totOccasionali").html(totaleOccasionali);
}
...
function Conferma() {
    if (confirm('Confermi operazione?')) {
        ...
        var totRecords = $("#totDipendenti").html();
        totRecords = parseInt(totRecords);
        var numPagine = totRecords / righePerPagina;
        var dati = GetDatiTabella();
        var paginaDiProvenienza = parseInt($("#currentPage").html());
        var paginaDiDestinazione = paginaDiProvenienza;       
        CambiaPagina(numPagine, paginaDiProvenienza, paginaDiDestinazione, dati);        
        return true;
    }
    else {
